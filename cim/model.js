...
function cimModel() {
    ...
    // This is a 'private' function (not visible in the model object).
    //options Added by SS.
    function createTerminal(object, options) {
        let term = cimObject("cim:Terminal", options);
    ...
    let model = {
        ...
        updateModel(rdf) {
            ...
            for (let obj of objs) {
                ...
                if (typeof (actObj) === "undefined") {
                    actObj = model.createObject(obj.nodeName, {
                        uuid: idValue
                    });
                }
            }
        ...
        insertElements(objs, objsProps) {
            ...
            for (let obj of objs) {
                ...
                if (typeof (actObj) === "undefined") {
                    ...
                    if (objsProps != undefined) {
                        actObj = model.createObject(obj.nodeName, objsProps[l]);
                    } else {
                        actObj = model.createObject(obj.nodeName, {
                            uuid: idValue
                        });
            ...
            for (let obj of objs) {
                ...
                actObj.px = obj.px;
                actObj.py = obj.py;
                actObj.rotation = obj.rotation;
                actObj.lineData = obj.lineData;

                if (actObj.nodeName === "cim:ACLineSegment" || actObj.nodeName == "cim:BusbarSection" ||
                    actObj.nodeName === "cim:ConnectivityNode") {
                    if (objsProps != undefined) {
                        actObj.lineData = objsProps[l]["lineData"];
                    }
                }

                model.addToActiveDiagram(actObj, actObj.lineData);

                if (actObj.nodeName === "cim:ConnectivityNode") {

                    if (objsProps != undefined) {

                        let busbar = this.getBusbar(obj);
                        if (busbar !== null) {} else {
                            for (let i = 0; i < objsProps[l]["connectivityNodeTerminalsUUID"].length; i++) {
                                let schemaLinkName = "cim:" + "Terminal.ConnectivityNode";
                                let termObj = this.getObject(objsProps[l]["connectivityNodeTerminalsUUID"][i]);

                                if (termObj != undefined) {
                                    this.setLink(termObj, schemaLinkName, actObj);
                                }
                            }
                        }
                    }
        ...
        createObject(type, options) {
            ...
            let subOptions = {};
            if ((typeof (options) !== "undefined") && options.hasOwnProperty("term1.uuid")) {
                subOptions["uuid"] = options["term1.uuid"];
                term1 = createTerminal(newElement, subOptions);
            } else {
                term1 = createTerminal(newElement);
            }
            ...
            if (type === "cim:ACLineSegment" ||
                ...
                subOptions = {};
                if ((typeof (options) !== "undefined") && options.hasOwnProperty("term2.uuid")) {
                    subOptions["uuid"] = options["term2.uuid"];
                    term2 = createTerminal(newElement, subOptions);
                } else {
            ...
            if (type === "cim:PowerTransformer") {
                if (windNum === 3) {
                    subOptions = {};
                    if ((typeof (options) !== "undefined") && options.hasOwnProperty("term3.uuid")) {
                        subOptions["uuid"] = options["term3.uuid"];
                        term3 = createTerminal(newElement, subOptions);
                    } else {
        ...
        deleteObjects(objects) {
            ...
            function checkRelatedObject(object, related) {
                ...
                // discrete value
                if (object.nodeName === "cim:Discrete") {
                    if (related.nodeName === "cim:DiscreteValue") {
                        return true;
                    }
                }
                // remote source
                if (object.nodeName === "cim:DiscreteValue") {
                    if (related.nodeName === "cim:RemoteSource") {
                        return true;
                    }
                }
                // analog value
                if (object.nodeName === "cim:Analog") {
                    if (related.nodeName === "cim:AnalogValue") {
                        return true;
                    }
                }
                // remote source
                if (object.nodeName === "cim:AnalogValue") {
                    if (related.nodeName === "cim:RemoteSource") {
                        return true;
                    }
                }
        ...
        clear() {
            data.all = null;
            model.fileName = null;
            model.activeDiagramName = null;
            model.activeDiagram = null;
            dataMap = new Map();
            linksMap = new Map();
        }
